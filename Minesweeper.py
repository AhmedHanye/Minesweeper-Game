...
from tkinter import messagebox # this if for the messagebox that will show up when you win
...
def toggle_fullscreen(): # this function is for the full screen
    root.attributes("-fullscreen", not root.attributes("-fullscreen")) # this is the attribute of the full screen
...
menu_bar.add_command(label="New Game", command=new_game) # this is for adding the new game button
menu_bar.add_command(label="Full Screen", command=toggle_fullscreen) # this is for adding the full screen button
menu_bar.add_command(label="Help" ,command=show_help) # this is for adding the help button
menu_bar.add_command(label="Exit", command=root.quit) # this is for adding the exit button
root.config(menu=menu_bar) # this is for adding the menu bar to the main window
...
## Create cells frame
cells_frame = Frame(root, bg="white") # this is the frame that contains the cells 
cells_frame.pack(side=LEFT, fill=BOTH, expand=True) # this is the position of the frame

Cells = [] # this is the list that contains the cells
A = [0] * 5 + [1] * 20 # this is the list that contains the numbers of the cells
...
def create_cells(): # this function is for creating the cells
    ...
    for row in range(5): 
        for column in range(5):
            O = choice(B) # this is for choosing a random number from the list B
            cell = Label(cells_frame, text=str(O), borderwidth=1, relief="solid", font=(
                "Arial", 16), width=2, height=1, bg="white") 
            B.remove(O) # this is for removing the number that we chose from the list B
            Cells.append(cell) # this is for adding the cell to the list Cells
            cell.grid(row=row, column=column, sticky="nsew") # this is for the position of the cells
            cells_frame.grid_columnconfigure(column, weight=1) # this is for adapting the size of the cells
            cells_frame.grid_rowconfigure(row, weight=1) # this is for adapting the size of the cells
            cell.bind("<Button-1>", toggle_cell) # this is for binding the left click to the toggle_cell function
            cell.bind("<Button-3>", toggle_cell) # this is for binding the right click to the toggle_cell function
            cell.config(fg="white", bg="white") # this is for making the text invisible
